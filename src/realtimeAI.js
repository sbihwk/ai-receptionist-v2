...
function buildSystemPrompt() {
  ...
  const name = businessConfig.name || 'our company';
  const hours = businessConfig.hours || 'business hours';
  const areas = businessConfig.serviceAreas || 'local areas';
  const phone = businessConfig.phone || '';

  let prompt = 'You are ' + agentName + ', a warm and professional receptionist for ' + name + '. You answer calls, book appointments, and help customers.\n\n';
  prompt += 'BUSINESS DETAILS:\n';
  prompt += '- Business: ' + name + '\n';
  prompt += '- Hours: ' + hours + '\n';
  prompt += '- Service Areas: ' + areas + '\n';
  prompt += '- Phone: ' + phone + '\n';
  prompt += '- Services: ' + services + '\n';
  if (faq) { prompt += '\nFAQ:\n' + faq + '\n'; }
  prompt += '\nHOW TO HANDLE CALLS:\n';
  prompt += '1. Greet warmly and ask how you can help\n';
  prompt += '2. Answer service or pricing questions from the details above\n';
  prompt += '3. If unsure say: Let me have our team follow up on that\n';
  prompt += '4. To book: collect name, phone, address, service needed\n';
  prompt += '5. Always call check_calendar before offering any time slots\n';
  prompt += '6. Confirm booking details out loud before saving\n';
  prompt += '7. If caller is upset: say I completely understand, let me take care of this\n';
  prompt += '8. If caller wants human: call transfer_to_human immediately\n';
  prompt += '9. After booking: confirm slot, thank them warmly\n';
  prompt += '\nSPEAKING STYLE:\n';
  prompt += '- Max 2 short sentences per response\n';
  prompt += '- Sound like a real warm human receptionist\n';
  prompt += '- Never say you are AI, a bot, or an assistant\n';
  prompt += '- No asterisks, markdown, bullet points, or internal thinking\n';
  prompt += '- Speak only what you would say on a phone call\n';
  prompt += '- If caller speaks Hindi or Hinglish, respond naturally in the same mix\n';
  return prompt;
}
  ...
  ws.on('message', async (data) => {
    try {
      ...
      if (msg.event === 'start') {
        ...
        callSid = msg.start.callSid || msg.start.customParameters && msg.start.customParameters.callSid;
        callerPhone = (msg.start.customParameters && msg.start.customParameters.callerPhone) || msg.start.from || 'unknown';
      ...
      geminiWs.send(JSON.stringify({
        setup: {
          ...
          generation_config: {
            ...
            speech_config: { voice_config: { prebuilt_voice_config: { voice_name: process.env.AGENT_VOICE || 'Aoede' } } }
          },
      ...
      try {
        ...
        if (event.setupComplete !== undefined) {
          ...
          for (const chunk of audioBuffer) {
            if (geminiWs.readyState === WebSocket.OPEN) {
              geminiWs.send(JSON.stringify({ realtime_input: { media_chunks: [{ mime_type: 'audio/pcm;rate=16000', data: chunk }] } }));
            }
          }
          audioBuffer = [];
        ...
        if (event.serverContent && event.serverContent.modelTurn && event.serverContent.modelTurn.parts) {
          for (const part of event.serverContent.modelTurn.parts) {
            if (part.inlineData && part.inlineData.mimeType && part.inlineData.mimeType.includes('audio') && ws.readyState === WebSocket.OPEN) {
